...
function shiftDown(arr, parentIndex, len) {
    ...
    while (childIndex < len) {
        // childIndex + 1 < len 说明右孩子存在， 取较大的（因为父节点要大于孩子节点）
...
function buildHeap(arr) {
    // 构建大顶堆不需要遍历所有元素，只需要从后往前遍历有孩子节点的父节点就行
    // 只要每一个有孩子节点的节点符合大顶堆定义，那么整棵树一定是大顶堆（这个思想很好理解吧）
...
function heapSort(arr) {
    ...
    for (let i = arr.length - 1; i > 0; i--) {
        // 遍历，因为已经是大顶堆了，所以根元素一定是最大的，直接和最后一个元素交换，
        swap(arr, 0, i)
